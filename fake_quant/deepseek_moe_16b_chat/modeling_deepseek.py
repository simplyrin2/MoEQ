...
class DeepseekMLP(nn.Module):
    def __init__(self, config, hidden_size = None, intermediate_size = None):
        ...
        self.gate_proj.cur_sample = 0
        self.up_proj.cur_sample = 0
        self.down_proj.cur_sample = 0

    def forward(self, x, cur_sample=0):
...
class DeepseekMoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.save_expert_cnt = False
        self.top1_cnt = torch.zeros(4,64)
        self.top2_cnt = torch.zeros(4,64)
        self.top3_cnt = torch.zeros(4,64)
        self.top4_cnt = torch.zeros(4,64)
        self.top5_cnt = torch.zeros(4,64)
        self.top6_cnt = torch.zeros(4,64)
    def clear_expert_cnt(self):
        self.top1_cnt = torch.zeros(4,64)
        self.top2_cnt = torch.zeros(4,64)
        self.top3_cnt = torch.zeros(4,64)
        self.top4_cnt = torch.zeros(4,64)
    ...
    def forward(self, hidden_states, cur_sample=0):
        identity = hidden_states
        orig_shape = hidden_states.shape
        topk_idx, topk_weight, aux_loss = self.gate(hidden_states)
        if self.save_expert_cnt:
            for m in range(topk_idx.shape[0]):
                idx = m//orig_shape[1]
                self.top1_cnt[idx,topk_idx[m][0]] = self.top1_cnt[idx,topk_idx[m][0]]+1
                self.top2_cnt[idx,topk_idx[m][1]] = self.top2_cnt[idx,topk_idx[m][1]]+1
                self.top3_cnt[idx,topk_idx[m][2]] = self.top3_cnt[idx,topk_idx[m][2]]+1
                self.top4_cnt[idx,topk_idx[m][3]] = self.top4_cnt[idx,topk_idx[m][3]]+1
                self.top5_cnt[idx,topk_idx[m][4]] = self.top5_cnt[idx,topk_idx[m][4]]+1
                self.top6_cnt[idx,topk_idx[m][5]] = self.top6_cnt[idx,topk_idx[m][5]]+1
        ...
        if self.training:
            ...
            y = self.moe_infer(hidden_states, flat_topk_idx, topk_weight.view(-1, 1), cur_sample).view(*orig_shape)
        if self.config.n_shared_experts is not None:
            y = y + self.shared_experts(identity)
        return y
    
    @torch.no_grad()
    def moe_infer(self, x, flat_expert_indices, flat_expert_weights, cur_sample=0):
        ...
        for i, end_idx in enumerate(tokens_per_expert):
            ...
            expert.gate_proj.cur_sample = cur_sample
            expert.up_proj.cur_sample = cur_sample
            expert.down_proj.cur_sample = cur_sample
...
class DeepseekDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states,cur_sample)
        hidden_states = residual + hidden_states
