...
def get_hadK(n, transpose=False):
    hadK, K = None, None
    if n % 684 == 0:  # deepSeek-moe Shared Expert (10944 = 684 * 16)
        assert (is_pow2(n // 684))
        K = 684
        hadK = get_had684().T if transpose else get_had684()
    ...
    elif n % 44 == 0:  # DeepSeek-MoE-Base FFN intermediate (1408 = 44 * 32)
        assert (is_pow2(n // 44))
        K = 44
...
def apply_exact_had_to_linear(module, had_dim=-1, output=False):
    ...
    if had_dim == -1:
        ...
        if output:
            ...
            # CRITICAL MATH FIX: Execute the missing block-diagonal inverse rotation
            n = W_.shape[1]
            W_ = fast_hadamard_transform.hadamard_transform(
...
def get_had684():
    """
    Generates a 684x684 Hadamard matrix using Paley Construction I.
    Relies on the prime p = 683. Fully vectorized for instant generation.
    """
    p = 683
    
    # 1. Find quadratic residues modulo 683
    idx = torch.arange(1, p, dtype=torch.long)
    residues = (idx * idx) % p
    
    # Create a fast boolean lookup mask
    is_residue = torch.zeros(p, dtype=torch.bool)
    is_residue[residues] = True
    
    # 2. Build the Jacobsthal matrix (Q) using broadcasting
    i = torch.arange(p).view(-1, 1)
    j = torch.arange(p).view(1, -1)
    diff = (i - j) % p
    
    # Q[i,j] = 1 if diff is a residue, -1 otherwise
    Q = torch.where(is_residue[diff], torch.tensor(1.0), torch.tensor(-1.0))
    Q.fill_diagonal_(0.0) # Diagonal must be 0
    
    # 3. Create S = Q - Identity
    S = Q - torch.eye(p, dtype=torch.float32)
    
    # 4. Assemble the final 684x684 Hadamard matrix (H)
    H = torch.ones((684, 684), dtype=torch.float32)
    H[1:, 1:] = S
