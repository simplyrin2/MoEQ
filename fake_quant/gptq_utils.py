...
class GPTQ:
    ...
    def add_batch(self, inp, out):
        # moe fix, if the expert received 0 tokens this batch, Hessian gets corrupted, so skip the update
        if inp.numel() == 0:
            return
...
def gptq_fwrd(model, dataloader, dev, args):
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(GPTQ Quant.) Layers"):
        ...
        # dynamic categorization of the linear layers into the 4 sequential steps
        seq1, seq2, seq3, seq4 = [], [], [], []
        for n in full.keys():
            if any(k in n for k in ['q_proj', 'k_proj', 'v_proj']):
                seq1.append(n)
            elif any(k in n for k in ['o_proj', 'out_proj']):
                seq2.append(n)
            elif any(k in n for k in ['up_proj', 'gate_proj', 'gate', 'w1', 'w3', 'fc1']):
                # catches standard MLP inputs, MoE routers, and all Expert inputs
                seq3.append(n)
            elif any(k in n for k in ['down_proj', 'w2', 'fc2']):
                # catches standard MLP outputs and all Expert outputs
                seq4.append(n)
                
        # filter out empty sequences (e.g., if a model doesn't have a certain projection)
        sequential = [seq for seq in [seq1, seq2, seq3, seq4] if len(seq) > 0]

        # process each sequential subset
        for names in sequential:
            ...
            for name in subset:
                ...
                # moe fix, MoE Router in 16-bit to prevent misrouting
                if name.endswith('.gate') or name == 'gate':
                    layer_weight_bits = 16
                    continue

                if args.w_bits_down_proj is not None and any(k in name for k in ['down_proj', 'w2', 'fc2']):
        ...
        # pass inputs through the fully quantized layer to prep inputs for the next layer
        for j in range(args.nsamples):
            outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask, position_ids=position_ids)[0]

        # memory Cleanup before the next layer loop
        layers[i] = layer.cpu()
        del layer
        del gptq
        torch.cuda.empty_cache()
