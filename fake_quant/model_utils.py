...
try:
    DEEPSEEK_MODEL = transformers.models.deepseek.modeling_deepseek.DeepseekForCausalLM
    DEEPSEEK_LAYER = transformers.models.deepseek.modeling_deepseek.DeepseekDecoderLayer
except AttributeError:
    import deepseek_moe_16b_chat.modeling_deepseek
    ...
    logging.warning(f'Failed to import DeepSeek model: {e}')
...
try:
    ...
    import mistral_7b.modeling_mistral
    MISTRAL_MODEL = mistral_7b.modeling_mistral.MistralForCausalLM
    MISTRAL_LAYER = mistral_7b.modeling_mistral.MistralDecoderLayer
except Exception as e:
    logging.warning(f'Failed to import Mistral model: {e}')
...
def get_rope_function_name(model):
    ...
    if 'llama' in model_str or 'mixtral' in model_str or 'deepseek' in model_str:
        return "apply_rotary_pos_emb"
    raise NotImplementedError(f"RoPE function not defined for {type(model)}")
...
def get_deepseek(model_name, hf_token):
    torch.nn.init.kaiming_uniform_ = skip
    torch.nn.init.uniform_ = skip
    torch.nn.init.normal_ = skip
    model = deepseek_moe_16b_chat.modeling_deepseek.DeepseekForCausalLM.from_pretrained(
        model_name,
        torch_dtype='auto',
        use_auth_token=hf_token,
        low_cpu_mem_usage=True,
...
def get_model(
    ...
    if 'llama' in model_name_lower:
        ...
        return get_deepseek(model_name, hf_token)
    else:
...
def get_embeddings(model, model_type) -> list[torch.nn.Module]:
    if model_type in [LLAMA_MODEL, DEEPSEEK_MODEL, MISTRAL_MODEL]:
...
def get_pre_head_layernorm(model, model_type):
    ...
    elif model_type in [DEEPSEEK_MODEL, MISTRAL_MODEL]:
        pre_head_layernorm = model.model.norm
    else:
        raise ValueError(f'Unknown model type {model_type}')
