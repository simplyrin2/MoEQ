...
class ActQuantizer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # moe fix, do not attempt to quantize empty expert inputs
        if self.bits == 16 or x.numel() == 0:
            return x

        self.find_params(x)  # Calculate quantization parameters for the first time

        if self.residual:
            if self.sym:
                tmp_q = sym_quant_dequant(x, self.scale, self.maxq).to(x_dtype)

                self.find_params(x - tmp_q)  # Recalculate parameters for the residual
    ...
    def find_params(self, x):
        ...
        if self.groupsize > 0:
            ...
            # utils.cleanup_memory(verbos=False)    # QuaRot source code, do not add as it will increase inference time by 10x
            return
...
class ActQuantWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.quantizer.bits < 16:  # Quantize, if needed
            # self.quantizer.find_params(x)  # QuaRot source code, now modified in the quantizer.forward function
        ...
        if self.out_quantizer.bits < 16:  # Quantize the output, if needed
            # self.out_quantizer.find_params(x) # QuaRot source code, now modified in the quantizer.forward function
