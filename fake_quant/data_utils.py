...
def get_wikitext2(nsamples, seed, seqlen, model, hf_token, eval_mode=False):
    ...
    if eval_mode:
        testdata = datasets.load_dataset('wikitext', 'wikitext-2-raw-v1', split='test', trust_remote_code=True)
        testenc = tokenizer("\n\n".join(testdata['text']), return_tensors='pt')
        return testenc
    else:
        traindata = datasets.load_dataset('wikitext', 'wikitext-2-raw-v1', split='train', trust_remote_code=True)
...
def get_c4_new(nsamples, seed, seqlen, model, hf_token=None, eval_mode=False):
    ...
    if eval_mode:
        ...
        traindata = datasets.load_dataset(
            'allenai/c4', 'en', split='train', trust_remote_code=True)
